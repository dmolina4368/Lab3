...
# 1.	Moran’s I is calculated by dividing the features by the spatial weights sum, then dividing the cross-products and the squared deviation sums.

# 2.	A spatially lagged variable is the average of the values of the variable and its neighbors.

# 3.	My analysis and the formalized W are different because the W is an advanced technique of calculating the spatial difference.

# 4.	That would mean an observation is the only one with a high value while its surrounded by variables with low values. It detects which variables stand out in data.
